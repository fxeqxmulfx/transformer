...
§4 of arXiv:1904.03590v4: Theorem 4.1 (`mainthm_change_beta1`).  Lemma 4.4
is `Section4_MainLemma`, Corollary 4.5 is `Section4_Corollary`.
...
theorem mainthm_lambda {S : Setup d} {F : Set (Vec d)} {D G : ℝ} (hS : IsOnlineConvex S F D G)
  ...
  have hβt : ∀ t, 1 ≤ t → 0 ≤ S.β₁ t ∧ S.β₁ t ≤ β₁ := fun t _ => by
    rw [hb]
    exact ⟨mul_nonneg hβ₁ (pow_nonneg hl.le _),
      mul_le_of_le_one_right hβ₁ (pow_le_one₀ hl.le hl'.le)⟩
  ...
  have h2 : ∑ i, ∑ t ∈ Icc 1 T, S.α t / (1 - S.β₁ 1) * S.m amsgradRule t i ^ 2 /
      ...
      α * Real.sqrt (Real.log T + 1) /
        ((1 - β₁) ^ 2 * Real.sqrt (1 - S.β₂) * (1 - β₁ / Real.sqrt S.β₂)) *
          ∑ i, S.gnorm amsgradRule T i := by
    rw [hb1]
    exact eqsecond_le (R := amsgradRule) (fun _ _ _ => le_sup_right) hα.le hαt hβt hβ₁'
      hβ₂ hβ₂' hγ T
...
/-- The hypotheses of Theorem 4.1, in both settings, are satisfiable: the zero
cost on `[-1, 1]`, `α = 1`, `β₁ = 0`, `λ = 1/2`, `β₂ = 1/2`. -/
example :
    ...
    IsOnlineConvex S (Set.Icc (fun _ => -1) (fun _ => 1)) 2 0 ∧ (0 : ℝ) < 1 ∧
      ...
      (∀ t, S.β₁ t = 0 / t) ∧ 0 < S.β₂ ∧ S.β₂ < 1 ∧ 0 / Real.sqrt S.β₂ < 1 :=
