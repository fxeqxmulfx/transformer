...
and a remainder `r_j(t)`, the bound `e:boundrj` on the remainder, and the
...
* `e:boundrj`, `|r_j(t)| ≤ Ce^{t(λ₁+|λ₂|)}`, is unproved.  The source derives
  it from `l:nottoofassst` through the three cross terms `P₁, P₂, P₃`, and
  those two growth bounds are carried here as explicit hypotheses, `hF` and
  `hG`, so that the statement says what it rests on.  The derivation also uses
  `π_ℱ(Ae^{tV}z_j(t)) = e^{λ₁t}π_ℱ(Az_j(t))`, which the source asserts in
  passing and which needs `A` to respect the splitting; that is why `hF` and
  `hG` alone do not close the proof here.  `|λ₂|` is the parameter `mu`,
  constrained only by `0 ≤ mu < λ₁`, which is what `d:goodmulti` gives.
...
/-- **Equation (e:boundrj).**  `|r_j(t)| ≤ Ce^{t(λ₁+|λ₂|)}` for all `t ≥ 0`
and `j ∈ [n]`.

Not proved here; see the file header for what the source's derivation uses
beyond `hF` and `hG`.

Source: arXiv:2305.05465v6, `e:boundrj`. -/
theorem exists_bound_scoreRemainder (Q K V A proj projG : ParamMatrix d)
    (F G : Submodule ℝ (EucSpace d)) (lam mu : ℝ)
    (hQKV : IsGoodTripleMulti Q K V F G lam) (hA : IsAttentionRoot Q K A)
    (hproj : IsProjOnto proj F G) (hprojG : IsProjOnto projG G F)
    (hmu : 0 ≤ mu) (hmulam : mu < lam)
    (Z : ℝ → Idx n → EucSpace d) (hZ : RescaledDynamics Q K V Z)
    (hF : ∃ C : ℝ, 0 < C ∧ ∀ t : ℝ, 0 ≤ t → ∀ j : Idx n,
      ‖proj (A (expTime V t (Z t j)))‖ ≤ C * Real.exp (lam * t))
    (hG : ∃ C : ℝ, 0 < C ∧ ∀ t : ℝ, 0 ≤ t → ∀ j : Idx n,
      ‖projG (A (expTime V t (Z t j)))‖ ≤ C * Real.exp (mu * t)) :
    ∃ C : ℝ, 0 < C ∧ ∀ t : ℝ, 0 ≤ t → ∀ i j : Idx n,
      |scoreRemainder A V proj lam Z t i j| ≤ C * Real.exp ((lam + mu) * t) := by
  sorry

/-- The hypotheses of `exists_bound_scoreRemainder` are satisfiable: the
triple `(I_d, I_d, I_d)` with `ℱ = ℝ^d`, `𝒢 = 0`, both projections the
identity on their own factor, `μ = 1/2 < 1 = λ₁`, and the stationary
configuration, whose two growth bounds hold with `C = ‖z‖ + 1`. -/
example (d : ℕ) (z : EucSpace d) :
    IsGoodTripleMulti (1 : ParamMatrix d) 1 1 ⊤ ⊥ 1 ∧
      IsAttentionRoot (1 : ParamMatrix d) 1 1 ∧
      IsProjOnto (1 : ParamMatrix d) ⊤ ⊥ ∧ IsProjOnto (0 : ParamMatrix d) ⊥ ⊤ ∧
      (0 : ℝ) ≤ 1 / 2 ∧ (1 : ℝ) / 2 < 1 ∧
      RescaledDynamics (n := n) (1 : ParamMatrix d) 1 1 (fun _ _ => z) := by
  refine ⟨isGoodTripleMulti_one d, isAttentionRoot_id d, isProjOnto_one d,
    ⟨fun _ => Submodule.zero_mem _, fun x hx => ?_, fun _ _ => rfl⟩, by norm_num, by norm_num,
