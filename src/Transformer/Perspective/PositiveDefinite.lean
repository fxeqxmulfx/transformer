/-
# The kernel `e^{β x·y}` is positive definite on the sphere

`∫∫ e^{β x·y} db(y) da(x) = Σ_k β^k/k! Σ_I m_I(a) m_I(b)`
(`hasSum_integral_exp_inner`), the two-measure form of the expansion
`hasSum_interactionEnergy` runs on, with the moments
`m_I(μ) = ∫ x_{I 0} ⋯ x_{I (k-1)} dμ(x)`.
...
and a series of this form whose sum is `≤ 0` forces `μ₁ = μ₂`
(`eq_of_hasSum_sq_moment_sub_nonpos`), moments determining the measure
(`SphereMoments`).
...
`μ₁ = μ₂` (`eq_of_integral_exp_inner_eq`).  That is the injectivity of
`μ ↦ f^μ` in `lem: quadpol` of arXiv:2601.21366v2, at the level of measures
(`Perceptron.Transform` reads it off).  The source proves it through the
Funk–Hecke formula instead: the spherical-harmonic coefficients of `f^ν` are
`λ_j(β) m_{jℓ}(ν)` with `λ_j(β) > 0`.  Both routes end in the same
Stone–Weierstrass step; this one needs no spherical harmonics, which Mathlib
does not have.
...
/-- **A moment series of squares with sum `≤ 0` forces `μ₁ = μ₂`.**  For
`β > 0`, the terms of `Σ_k β^k/k! Σ_I (m_I(μ₁) - m_I(μ₂))²` are nonnegative,
so a sum `≤ 0` makes every one of them vanish, hence every moment agrees
(`measure_eq_of_integral_mono_eq`).  The sum is
`T(μ₁,μ₁) - T(μ₁,μ₂) - T(μ₂,μ₁) + T(μ₂,μ₂)` by `hasSum_integral_exp_inner`:
this is the strict positive definiteness of the kernel `e^{β x·y}`.

Source: arXiv:2601.21366v2, proof of `lem: quadpol` (injectivity). -/
theorem eq_of_hasSum_sq_moment_sub_nonpos (β : ℝ) (hβ : 0 < β) (μ₁ μ₂ : Measure (SSphere d))
    [IsFiniteMeasure μ₁] [IsFiniteMeasure μ₂] {s : ℝ} (hs : s ≤ 0)
    (h : HasSum (fun k : ℕ => β ^ k / k ! * ∑ I : Fin k → Fin d,
      ((∫ x, mono I (x : EucSpace d) ∂μ₁) - ∫ x, mono I (x : EucSpace d) ∂μ₂) ^ 2) s) :
    μ₁ = μ₂ := by
  have hnn : ∀ k : ℕ, 0 ≤ β ^ k / k ! * ∑ I : Fin k → Fin d,
      ((∫ x, mono I (x : EucSpace d) ∂μ₁) - ∫ x, mono I (x : EucSpace d) ∂μ₂) ^ 2 :=
    fun k => by positivity
  rw [le_antisymm hs (h.nonneg hnn), hasSum_zero_iff_of_nonneg hnn] at h
...
/-- The hypotheses of `eq_of_hasSum_sq_moment_sub_nonpos` are satisfiable: `β = 1`,
`μ₁ = μ₂ = 0`, whose moment differences all vanish, and `s = 0`. -/
example : (0 : ℝ) < 1 ∧ (0 : ℝ) ≤ 0 ∧
    HasSum (fun k : ℕ => (1 : ℝ) ^ k / k ! * ∑ I : Fin k → Fin 1,
      ((∫ x, mono I (x : EucSpace 1) ∂(0 : Measure (SSphere 1))) -
        ∫ x, mono I (x : EucSpace 1) ∂(0 : Measure (SSphere 1))) ^ 2) 0 :=
  ⟨one_pos, le_rfl, by simp⟩
...
theorem eq_of_integral_exp_inner_eq (β : ℝ) (hβ : 0 < β) (μ₁ μ₂ : Measure (SSphere d))
  ...
  refine eq_of_hasSum_sq_moment_sub_nonpos β hβ μ₁ μ₂ le_rfl ?_
  convert h3 using 1
  funext k
