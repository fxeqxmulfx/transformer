...
- `RASPL.Attention.argmax_shift` — hard attention against a shifted score
  ...
  timer is built from, and it is proved rather than assumed.
...
- **A connection between head counts and a trained policy.**  `RASP.Expr.heads`
  ...
  bridge between the two is T1 below, and it is not written.  Until it is, the
  head counts say nothing about a model that was trained rather than compiled.
...
Four, in order of what they buy.  Everything in §1 is about *one head in
...
- [ ] **T3 — attention is necessary for the event memory.**
      ...
      Cheap, and it is the only place the premise of the task becomes a
      theorem: it shows attention is needed for *memory over events*.  It says
      nothing about generalization across maps, which is T4's business and
      stays a conjecture there.
...
- [ ] **T4 — equivariance, in place of the generalization conjecture.**
      ...
      coordinates is really about: for the group `G` generated by time
      translation, yaw rotation and permutation of simultaneous events,
      `eval e (g · x) = g · eval e x` for every `e` built from the
      relative-only primitives.
      ...
      *precisely* the primitive that breaks it, and every other constructor
      ...
      closure property of a fragment, and gives the token encoding something
      to check against.
