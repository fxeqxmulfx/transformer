...
| `GGUF.Accumulator` | a binary16 accumulator stops counting at `2^{11}` times its increments: the flash-attention output decays like `1/n` |
...
import Transformer.GGUF.Accumulator
