...
* Lemma 3.1 is stated for Algorithm 1; §5 applies it to AdamX "similarly".
  Its proof uses of `v̂_t` only that `m_{t,i} = 0` wherever `v̂_{t,i} = 0`,
  which every rule with `v_t ≤ v̂_t` has when `0 < β₂ < 1`
  (`m_eq_zero_of_vhat`), so it is stated once, for every rule with that
  property, `prepare_lem`; `amsgradRule` is such a rule, `le_amsgradRule`.
  It needs `0 < α_t` and `0 ≤ β_{1,t} ≤ β₁ < 1`, which the source assumes
  throughout.
...
/-- **Lemma 3.1.**  For a rule with `m_{t,i} = 0` wherever `v̂_{t,i} = 0`, `t ≤ T`
(by `m_eq_zero_of_vhat`, every rule with `v_t ≤ v̂_t` when `0 < β₂ < 1`:
AMSGrad, and AdamX of §5), under the standing assumptions, for all `T ≥ 1`
and `x* ∈ F`,
...
theorem prepare_lem {S : Setup d} {F : Set (Vec d)} {D G : ℝ} (hS : IsOnlineConvex S F D G)
    {R : Rule d} (hα : ∀ t, 1 ≤ t → 0 < S.α t)
    (hβ₁ : ∀ t, 1 ≤ t → 0 ≤ S.β₁ t ∧ S.β₁ t ≤ S.β₁ 1) (hβ₁' : S.β₁ 1 < 1)
    {T : ℕ} (hT : 1 ≤ T) (hz : ∀ n ≤ T, ∀ i, Real.sqrt (S.vhat R n i) = 0 → S.m R n i = 0)
  ...
  have hstep : ∀ t ∈ Icc 1 T, S.f t (S.x R t) - S.f t xstar ≤
    ...
    have hst := step_ineq hS ht.1 hαt hb0 (by linarith) (hz t ht.2) hxstar
    ...
    have hC : S.β₁ t / (1 - S.β₁ t) * S.m R (t - 1) i * (xstar i - S.x R t i) ≤
      ...
      have hy := young (m := m) (z := z) hαs (Real.sqrt_nonneg _) (hz s (by omega) i)
...
example :
    ...
    IsOnlineConvex S (Set.Icc (fun _ => -1) (fun _ => 1)) 2 0 ∧ (∀ t, 1 ≤ t → 0 < S.α t) ∧
      (∀ t, 1 ≤ t → 0 ≤ S.β₁ t ∧ S.β₁ t ≤ S.β₁ 1) ∧ S.β₁ 1 < 1 ∧ 1 ≤ 1 ∧
      (∀ n ≤ 1, ∀ i, Real.sqrt (S.vhat amsgradRule n i) = 0 → S.m amsgradRule n i = 0) ∧
      (0 : Vec 1) ∈ Set.Icc (fun _ => -1) (fun _ => 1) :=
  ⟨isOnlineConvex_zero _ _ _, fun _ _ => one_pos, fun _ _ => ⟨le_rfl, le_rfl⟩,
    by simp [zeroSetup], le_rfl,
    fun _ _ _ h => m_eq_zero_of_vhat le_amsgradRule (by norm_num [zeroSetup])
